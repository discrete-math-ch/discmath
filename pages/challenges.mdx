...
<Problem title="Two be or not two be" difficulty={5} relevance={0} source="Yannick Funke, Max Obreiter">
    ...
    <div label="answer">
        ...
        $$
            ...
            \end{aligned}
        $$
        Since $c \ge 2 \iff c - 2 \ge 0$, we have $(c - 2) \in \mathbb N$. This means that $c \ge c - 2$, which is a contradiction to the minimality of $c$. Therefore, $c \le 1$.
...
<Problem title="Counting the Uncountable" difficulty={3} relevance={3} source="Yannick Funke">
    <div label="question">
        ...
        Prove that $~$ is an equivalence relation on Q (for a fixed $B$). Then prove that $(Q/\sim)$ is countable (for a fixed $B$).
    </div>
    <div label="hint">
        Recall that equivalence relations need to fullfill reflexivity,
        symmetry and transitivity.
        Note that the existence of a bijection between two sets implies that
        they are equinumerous.
    </div>
    <div label="hint">
        Use a set of representatives of $Q/~$
    </div>
    <div label="answer">
        **Reflexivity** <br/>
        Let $(A, f) \in Q$. Clearly $\mathrm{id}_A$ is a bijective function on A.
        $$
        f = \mathrm{id}_A \circ f \circ \mathrm{id}_A = \mathrm{id}_A^{-1} \circ f \circ \mathrm{id}_A \text{,}
        $$
        since $\mathrm{id}_A = \mathrm{id}_A^{-1}$.
        Thus by definition of $\sim$, $(A, f) \sim (A, f)$. Proving that $\sim$ is reflexive.

        **Symmetry** <br/>
        Let $(A_1, f_1), (A_2, f_2) \in Q$ s.t. (such that) $(A_1, f_1) \sim (A_2, f_2)$.
        Then there is a bijection $g: A_1 \to A_2$ s.t.: $f_1 = g^{-1} \circ f_2 \circ g$.
        $$
        \begin{aligned}
        f_2 &=  g \circ g^{-1} \circ f_2 \circ g \circ g^{-1} && g \circ g^{-1} = \mathrm{id}_A \\
            &= g \circ f_1 \circ g^{-1} && \text{substitute $f_1$}\\
            &= (g^{-1})^{-1} \circ f_1 \circ g^{-1} && (g^{-1}) ^{-1} = g\\
        \end{aligned}
        $$
        Using the fact that $g^{-1}$ is bijective wich we will prove in the following,
        this implies that $(A_2, f_2) \sim (A_1, f_1)$.

        From the script, we know that $g^{-1}$ is a function. It remains to show
        injectivity and surjectivity.

        $$
        \begin{aligned}
        &\text{$g$ is totally defined} \\
        &\implies \forall a \in A_1 \hspace{1em}\exists b \in A_2 \hspace{1em} a\ g\ b && \text{Def. totally defined}\\
        &\implies \forall a \in A_1 \hspace{1em}\exists b \in A_2 \hspace{1em} b\ g^{-1}\ a && \text{Def. inverse relation}\\
        &\implies \forall a \in A_1 \hspace{1em}\exists b \in A_2 \hspace{1em} g^{-1}(b) = a && \text{Def. function} \\
        &\implies \text{$g^{-1}$ is surjective} && \text{Def. surjective}
        \end{aligned}
        $$
        ...
        &\implies \forall a \in A_1 \hspace{1em}\forall b, b' \in A_2 \hspace{1em} a\ g\ b \land a\ g\ b' \rightarrow b = b'  && \text{Def. well defined}\\
            &\implies \forall a \in A_1 \hspace{1em}\forall b, b' \in A_2 \hspace{1em} b\ g^{-1}\ a \land b'\ g^{-1}\ a \rightarrow b = b'  && \text{Def. inverse relation}\\
            &\implies \forall a \in A_1 \hspace{1em}\forall b, b' \in A_2 \hspace{1em} g^{-1}(b) = a \land g^{-1}(b') = a \rightarrow b = b'  && \text{Def. function}\\
            &\implies \forall b, b' \in A_2 \hspace{1em} (\exists a \in A_1 \hspace{1em} g^{-1}(b) = a = g^{-1}(b')) \rightarrow b = b'  && \text{logic}\\
            &\implies \forall b, b' \in A_2 \hspace{1em} g^{-1}(b) = g^{-1}(b') \rightarrow b = b'  \\
            &\implies \forall b, b' \in A_2 \hspace{1em} b \neq b' \rightarrow g^{-1}(b) \neq g^{-1}(b') && \text{contraposition}\\
            &\implies \text{$g^{-1}$ is injective} && \text{Def. injective}
        \end{aligned}
        $$

    </div>
    <div label="takeaway">
        We used equivalence classes to categorize functions that behave the
        same while being defined on different sets of same cardinality.
        While there are uncountable many sets that have the same cardinality
        this abstraction allowed us to reduce the set of functions to a
        countable set.
    </div>
</Problem>
</Callout>
