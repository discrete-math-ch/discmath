...
3. <span className="relative group inline-block no-overflow"> <span className="block px-2 py-1 bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-md cursor-pointer"> Think about what would happen in the picture above if $h$ was injective! (*hover to reveal*)</span> <span className="absolute z-10 left-0 mt-2 hidden w-max px-4 py-2 bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 rounded-md sh adow-lg group-hover:block no-overflow">
  If $h$ was injective, then there would not be two different elements in $G$ which map to the same element in $H$. So we sort of would need the group $H$ to be at least as big (restricting ourselves to finite groups).  even we haven't defined the notion of subgroups yet, we could actually show that $\text{im} h$ is a subgroup of $H$. The intuition here is that we can sort of not *escape* $\text{im} h$ by *multiplying* with elements in $\text{im} h$,</span></span>
...
<details>
<summary>$h: \mathbb{Z}_{24} \to \mathbb{Z}_{18}$</summary>
...
Some of these properties go beyond of what you *should technically* know for the course. However, knowing these and having an intuition of where they come from, makes life so much easier when thinking about or constructing isomorphisms/homomorphisms which is definitely a main topic in this chapter. Additionally, you might be asked to prove properties like these in an exem ;).
</Callout>

We will consider the homomorphism $h : G \to H$, where $\langle G; *, \hat{}, e_G \rangle$ is a group and $\langle H; \star, \tilde{}, e_h \rangle$ is group.

| Property                                  | Intuitive Meaning                                                                 |
|-------------------------------------------|------------------------------------------------------------------------------------|
| $h(e_G) = e_H$                            | The identity in $G$ maps to the identity in $H$, meaning the structure's "base point" is preserved. |
| $h(g^{-1}) = h(g)^{-1}$                   | Inverses in $G$ map to inverses in $H$, so reversing elements in $G$ reverses them in $H$ as well.  |
| $h(g_1 \cdot g_2) = h(g_1) \cdot h(g_2)$  | Multiplication (or addition) is preserved, maintaining the group operation’s structure.             |
| $\ker(h) = \{g \in G \mid h(g) = e_H\}$   | The kernel tells us which elements of $G$ are "collapsed" into the identity in $H$, essentially measuring how much structure is “lost” under $h$. |
| $h(G)$ is a subgroup of $H$               | The image of $G$ under $h$ is itself a group within $H$, showing that $G$’s structure can partially "live" inside $H$. |
| $h$ is injective $\iff \ker(h) = \{e_G\}$ | If only the identity maps to the identity, then $h$ preserves distinct elements, showing that $h$ is an embedding of $G$ into $H$. |
...
Similarly, the **right coset** of $H$ with respect to $g$ is the set:
$$
Hg = \{ hg \mid h \in H \}.
$$
</Callout>
...
<div label="hint">
Write every element from $H$ as $g^k$ for some $k \in \mathbb{N}$ and take *some* minimum. Then use Euclidean Divison Theorem. 
</div>
<div label="answer">
